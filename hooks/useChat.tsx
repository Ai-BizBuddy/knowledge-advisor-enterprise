...
export const useChat = () => {
  ...
  const [connectionStatus, setConnectionStatus] = useState<'connected' | 'connecting' | 'error' | 'timeout'>('connected');
  ...
  const addWelcomeMessage = useCallback(() => {
    ...
    if (!content.trim() || selectedKBs.length === 0) {
      if (selectedKBs.length === 0) {
        const errorMessage: ChatMessage = {
          id: Date.now().toString(),
          type: 'assistant',
          content: 'กรุณาเลือก Knowledge Base อย่างน้อย 1 รายการก่อนที่จะถามคำถามครับ',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        };
    ...
    setIsTyping(true);
    setConnectionStatus('connecting');    try {
      ...
      if (chatResult.success) {
        ...
        if (chatResult.error?.includes('ใช้เวลานานเกินไป') || chatResult.error?.includes('timeout')) {
      ...
      const aiResponse: ChatMessage = {
        ...
        type: 'assistant',
        content: chatResult.success ? chatResult.content : (chatResult.error || 'ขออภัยครับ เกิดข้อผิดพลาดในการประมวลผล'),
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      ...
      const errorResponse: ChatMessage = {
        id: (Date.now() + 1).toString(),
        type: 'assistant',
        content: 'ขออภัยครับ เกิดข้อผิดพลาดในการเชื่อมต่อกับ AI Assistant กรุณาลองใหม่อีกครั้ง',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
