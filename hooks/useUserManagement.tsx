...
interface UseUserManagementActions {
  ...
  checkPermission: (userId: string, resource: string, action: string) => Promise<PermissionCheckResult>;
...
export interface UseUserManagement extends UseUserManagementState, UseUserManagementActions {}

const userManagementService = new UserManagementService({ useMockData: true }); // Set to false for production
...
export const useUserManagement = (): UseUserManagement => {
   ...
   * Create new user
   */
  const createUser = useCallback(async (userData: CreateUserInput): Promise<User | null> => {
    try {
      setLoading(true);
      setError(null);
   ...
   * Update user
   */
  const updateUser = useCallback(async (id: string, updates: UpdateUserInput): Promise<User | null> => {
    try {
      setLoading(true);
      setError(null);
   ...
   */
  const checkPermission = useCallback(async (userId: string, resource: string, action: string): Promise<PermissionCheckResult> => {
    try {
      setError(null);
      return await userManagementService.checkPermission(userId, resource, action as PermissionAction);
    } catch (error) {
   ...
   */
  const getCurrentUserSession = useCallback(async (userId: string) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get user session';
      setError(errorMessage);
