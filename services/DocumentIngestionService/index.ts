...
class DocumentIngestionService {
  ...
  private transformToApiError(error: TypedFetchError): TypedFetchError {
    ...
    if (error.status) {
      ...
      // Network error
      apiError.message =
        'No response from server. Please check if the ingestion service is running on localhost:5001';
      apiError.code = 'NETWORK_ERROR';
    } else if (error.isTimeoutError) {
      // Timeout error
      apiError.message =
        'Request timeout. The ingestion service may be overloaded';
  ...
  async syncDocuments(
    ...
    try {
      const response = await this.client.post<DocumentSyncResponse>(
        '/Documents/sync',
        request,
      );
  ...
  async getPendingDocuments(): Promise<PendingDocumentsResponse> {
    try {
      const response = await this.client.get<
        PendingDocumentsResponse | unknown[]
      >('/Documents/pending');
        ...
        return {
          documents: data.map((doc) => ({
            documentId:
              ...
              ((doc as Record<string, unknown>).documentId as string),
            name:
              ((doc as Record<string, unknown>).name as string) || 'Unknown',
            queuedAt:
              ((doc as Record<string, unknown>).queuedAt as string) ||
              new Date().toISOString(),
            priority:
              ((doc as Record<string, unknown>).priority as number) || 1,
          })),
          total: data.length,
        };
      }
  ...
  async getFailedJobs(): Promise<FailedJobsResponse> {
    try {
      const response = await this.client.get<FailedJobsResponse | unknown[]>(
        '/Jobs/failed',
      );
      const data = response.data;
      if (Array.isArray(data)) {
        return {
          jobs: data.map((job) => ({
            jobId:
              ((job as Record<string, unknown>).jobId as string) || 'unknown',
            documentId: (job as Record<string, unknown>).documentId as string,
            errorMessage:
              ((job as Record<string, unknown>).errorMessage as string) ||
              'Unknown error',
            failedAt:
              ((job as Record<string, unknown>).failedAt as string) ||
              new Date().toISOString(),
            retryCount:
              ((job as Record<string, unknown>).retryCount as number) || 0,
            maxRetries:
              ((job as Record<string, unknown>).maxRetries as number) || 3,
          })),
          total: data.length,
        };
      }
   ...
   */
  async getMultipleDocumentStatuses(
    documentIds: string[],
