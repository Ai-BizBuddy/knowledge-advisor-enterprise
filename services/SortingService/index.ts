...
export type SortField = "name" | "date" | "size" | "type" | "uploadedBy" | "status" | "chunk" | "lastUpdated";
...
class SortingService {
    ...
    sortDocuments(
        ...
        console.log(`[${this.serviceName}] Sorting ${documents.length} documents by ${sortConfig.field} (${sortConfig.order})`);
     ...
     */
    private compareSizes(a: number | null | undefined, b: number | null | undefined): number {
        const sizeA = a || 0;
        const sizeB = b || 0;
        return sizeA - sizeB;
    }
    ...
    validateSortConfig(config: SortConfig): boolean {
        ...
        return validFields.includes(config.field) && validOrders.includes(config.order);
    }
}
