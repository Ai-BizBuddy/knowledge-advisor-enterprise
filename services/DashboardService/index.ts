...
class DashboardService {
   ...
   */
  private async getKnowledgeBaseActivities(): Promise<ActivityItem[]> {
    try {
      const supabaseTable = createClientTable();
   ...
   */
  private async getDocumentActivities(): Promise<ActivityItem[]> {
    try {
      const supabaseTable = createClientTable();
      ...
      return data.map((doc) => {
        const kbId =
          doc.knowledge_base &&
          typeof doc.knowledge_base === 'object' &&
          'id' in doc.knowledge_base
