...
class LangflowChatService {
  ...
  constructor(config: LangflowChatServiceConfig = {}) {
    const baseUrl =
      ...
      'http://first-boy.ddns.net:7860';
    const chatPath =
      config.chatPath ||
      process.env.NEXT_PUBLIC_LANGFLOW_CHAT_PATH ||
      '/api/v1/run/07abd158-91dd-4c2c-a1af-9f7b586c217a';
  ...
  private extractResponseContent(response: LangflowChatResponse): string {
    try {
      ...
      console.error(
        `[${this.serviceName}] Error extracting response content:`,
        error,
      );
  ...
  async sendMessage(input: ChatInput): Promise<ChatResult> {
    ...
    try {
      ...
      const response = await this.client.post<LangflowChatResponse>(
        '?stream=false',
        request,
      );
